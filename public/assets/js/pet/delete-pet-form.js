export class DeletePetForm {
    _modalElement = null;
    _formElement = null;
    _deleteButtons = null;
    _cancelButton = null;
    _petIdInput = null;

    initialize() {
        this._cacheElements();
        this._addEventListeners();
    }

    _cacheElements() {
        this._modalElement = document.querySelector('.delete-pet-area');
        this._formElement = document.querySelector('.delete-pet-form');
        this._deleteButtons = document.querySelectorAll('.pet-actions .delete-button');
        this._cancelButton = this._formElement.querySelector('.cancel-button');
        this._petIdInput = this._formElement.querySelector('input[name="pet-id"]');
    }

    _addEventListeners() {
        if (this._deleteButtons) {
            this._deleteButtons.forEach(button => {
                button.addEventListener('click', (e) => this._openModalWithId(e));
            });
        }

        this._cancelButton.addEventListener('click', () => this._closeModal());
    }

    _openModalWithId(event) {
        const element = event.currentTarget;
        const petId = element.getAttribute('data-id');
        this._petIdInput.value = petId;
        this._modalElement.classList.remove('desactive');
    }

    _closeModal() {
        this._modalElement.classList.add('desactive');
    }
}